...
public class TipCommand extends CommandAction {
    ...
    public TipCommand(EmeraldQuest plugin) {
        emeraldQuest = plugin;
    }
       ...
       if(args.length == 2) {
                    ...
                    try {
                        if(emeraldQuest.countEmeralds(player) >= sendAmount) {
                            player.sendMessage(ChatColor.YELLOW+"Tipping " + args[0] + " Emerlads to "+args[1]+"...");
                                    ...
                                    scheduler.runTaskAsynchronously(emeraldQuest, new Runnable() {
                                        ...
                                        public void run() {
                                                ...
                                                if (((emeraldQuest.removeEmeralds(player,sendAmount))&&(emeraldQuest.addEmeralds(offlinePlayer.getPlayer(),sendAmount)))) {
                                                    player.sendMessage(ChatColor.GREEN + "Succesfully tipped " + sendAmount + " Emeralds to " + offlinePlayer.getName() + ".");
                                                    if (offlinePlayer.isOnline()) {
                                                        offlinePlayer.getPlayer().sendMessage(ChatColor.GREEN + "" + player.getName() + " just tipped you " + sendAmount + " Emeralds!");
                                                    }
                                                } else {
                                                    player.sendMessage(ChatColor.RED + "failed. Please try again in a few moments.");
                                                }
